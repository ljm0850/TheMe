...
public interface BoardRepository extends JpaRepository<Board,Integer> {
    ...
    @Query("select new BoardGroupListDto(b.themeIdx,b.userIdx,b.city,b.name,b.place,count (b.name),b.latitude,b.longitude,b.idx) from Board b group by b.name,b.createTime having b.userIdx In (?1) and b.themeIdx=?2 order by count(b.name) desc")
    ...
    @Query("select b from Board b group by b.place,b.createTime having b.themeIdx=?1")
    List<Board> findByThemeIdxGroupByPlace(int themeIdx);
    @Query("select b from Board b group by b.place,b.createTime having b.themeIdx=?1 and b.userIdx=?2")
    List<Board> findByThemeIdxAndUserIdxGroupByPlace(int themeIdx,int userIdx);
    @Query("select b from Board b group by b.userIdx,b.createTime having b.themeIdx=?1")
