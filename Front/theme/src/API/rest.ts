...
export default {
    ...
    User: {
        ...
        followtheme:(_theme_idx:string,  _target_user_idx:string) => HOST + USER + FOLLOW + `/${_theme_idx}/${_target_user_idx}`,
        cancelFollow:(_userThemeIdx:string) => HOST + USER + FOLLOW + `/${_userThemeIdx}`,
        ...
        isFollow : (_userIdx:number, _targetUserIdx:number, _themeIdx :number, ) => HOST + USER + `/isFollow/${_userIdx}/${_targetUserIdx}/${_themeIdx}`
    },
