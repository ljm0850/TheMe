<template>
  <div class="top-box">
    <button v-if="!state.isClicked" class="btn btn-light type-button border top-search"
      @click="selectSearch(0)">👨‍👦인물</button>
    <div v-if="state.isClicked && state.inputValue == 0">
      <div class="d-flex">
        ...
        <button class="btn btn-light type-button border btn-size-38 ms-2 me-2"
          @click="getSerchPerson(state.inputPersonText)">검색</button>
      </div>
    </div>
    <button v-if="!state.isClicked" class="btn btn-light type-button border btn-size-38 top-search"
      @click="selectSearch(1)">🎨테마</button>
    <div v-if="state.isClicked && state.inputValue == 1">
      <div class="d-flex">
        ...
        <button class="btn btn-light type-button border ms-2 me-2"
          @click="getSerchTheme(state.inputThemeText)">검색</button>
      </div>
    </div>
  </div> 
  ...
  <div v-else-if="!state.isSerched && state.inputThemeText.length >= 1" class="search-theme">
    <div v-for="theme in liveSearchTheme" :key="theme.name" class="search-theme-card"
...
.search-theme-card, .search-person-card {
  margin: 4px;
  border: 1px solid;
  border-color: beige;
  font-size: 20px;
